...
export async function calculateEndOfService(input: EndOfServiceInput): Promise<{ success: true; data: EndOfServiceOutput } | { success: false; error: string }> {
  ...
  const { worker, lastDayOfWork, reasonForTermination } = validation.data as { worker: Worker, lastDayOfWork: Date, reasonForTermination: 'termination' | 'resignation' };
  ...
  let finalGratuity = 0;
  if (reasonForTermination === 'termination') {
